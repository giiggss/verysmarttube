...
import com.liskovsoft.smartyoutubetv2.common.proxy.vmess.VmessProxyManager;
...
public class GeneralSettingsPresenter extends BasePresenter<Void> {
    ...
    private void appendInternetCensorship(AppDialogPresenter settingsPresenter) {
        ...
        appendVmessProxy(settingsPresenter, options);
    ...
    private void appendVmessProxy(AppDialogPresenter settingsPresenter, List<OptionItem> options) {
        VmessProxyManager vmessManager = new VmessProxyManager(getContext());

        // Toggle: enable/disable the built-in vmess proxy (starts/forwards local SOCKS)
        options.add(UiOptionItem.from(getContext().getString(R.string.enable_vmess_proxy),
                getContext().getString(R.string.enable_vmess_proxy_desc),
                option -> {
                    if (option.isSelected()) {
                        // Ask for the vmess link if none is configured yet.
                        if (vmessManager.getVmessLink().isEmpty()) {
                            SimpleEditDialog.show(getContext(),
                                    getContext().getString(R.string.paste_vmess_link),
                                    getContext().getString(R.string.paste_vmess_link_hint),
                                    "",
                                    newValue -> {
                                        String error = vmessManager.setEnabled(true, newValue);
                                        if (error != null) {
                                            MessageHelpers.showMessage(getContext(), error);
                                        }
                                        mRestartApp = error == null;
                                        return false;
                                    });
                        } else {
                            String error = vmessManager.setEnabled(true, null);
                            if (error != null) {
                                MessageHelpers.showMessage(getContext(), error);
                            }
                            mRestartApp = error == null;
                        ...
                        vmessManager.setEnabled(false, null);
                        mRestartApp = true;
                    }
                },
                vmessManager.isEnabled()));

        // Action: paste a vmess:// link (also lets user replace an existing one)
        options.add(UiOptionItem.from(getContext().getString(R.string.paste_vmess_link),
                option -> {
                    SimpleEditDialog.show(getContext(),
                            getContext().getString(R.string.paste_vmess_link),
                            getContext().getString(R.string.paste_vmess_link_hint),
                            vmessManager.getVmessLink(),
                            newValue -> {
                                vmessManager.setVmessLink(newValue);
                                if (vmessManager.isEnabled()) {
                                    // Re-apply with the new node.
                                    String error = vmessManager.setEnabled(true, newValue);
                                    if (error != null) {
                                        MessageHelpers.showMessage(getContext(), error);
                                    }
                                }
                                MessageHelpers.showMessage(getContext(), getContext().getString(R.string.vmess_link_saved));
                                mRestartApp = true;
                                return false;
                            });
                }));
